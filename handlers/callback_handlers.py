...
async def process_admin(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    if callback_data == 'check_reg':
        ...
        await db.update_registration_status(string_id=callback_data[2],
                                            admin_id=callback.from_user.id,
                                            reg_status="Accept")
        await state.set_state(User_states.registration_accepted)
        ...
        await callback.message.edit_text(text=information_panel, reply_markup=Admin_Keyboards.reg_process_keyboard(form_info_list[1], user_info_list[0]))
...
async def process_user(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    async def question_process(callback: types.CallbackQuery, state: FSMContext):
        await callback.message.edit_text(text='Выберете раздел', reply_markup=User_Keyboards.section_chose().as_markup)
